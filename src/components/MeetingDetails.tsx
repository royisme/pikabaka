...
const MeetingDetails: React.FC<MeetingDetailsProps> = ({ meeting: initialMeeting }) => {
    ...
    const filteredTranscript = useMemo(() =>
        meeting.transcript?.filter(entry =>
            !['system', 'ai', 'assistant', 'model'].includes(entry.speaker?.toLowerCase())
        ) || [],
        [meeting.transcript]
    );

    const allTranslated = filteredTranscript.length > 0 &&
        filteredTranscript.every((_, i) => translations[i]?.state === 'complete');

    useEffect(() => {
        return () => { translateAbortRef.current = true; };
    }, []);

    const handleTranslateEntry = async (index: number, entry: { speaker: string; text: string; timestamp: number }) => {
        setTranslations(prev => ({ ...prev, [index]: { text: '', state: 'pending' } }));
        try {
            const result = await window.electronAPI?.translateTranscriptSegment({
                segmentId: `dashboard_${meeting.id}_${index}`,
                text: entry.text,
                speaker: entry.speaker === 'user' ? 'user' : 'interviewer',
                speakerLabel: entry.speaker === 'user' ? 'Me' : 'Them',
                timestamp: entry.timestamp,
            });
            if (result?.success && result?.translatedText) {
                setTranslations(prev => ({ ...prev, [index]: { text: result.translatedText ?? '', state: 'complete' } }));
            } else {
                setTranslations(prev => ({ ...prev, [index]: { text: '', state: 'error' } }));
            }
        } catch {
            setTranslations(prev => ({ ...prev, [index]: { text: '', state: 'error' } }));
        }
        ...
        for (let i = 0; i < filteredTranscript.length; i++) {
            ...
            const shouldSkip = await new Promise<boolean>(resolve => {
                setTranslations(prev => {
                    resolve(prev[i]?.state === 'complete');
                    return prev;
                });
            });
            if (!shouldSkip) {
                await handleTranslateEntry(i, filteredTranscript[i]);
            }
        }
                        ...
                        <div className="flex items-center gap-3">
                            {activeTab === 'transcript' && (
                                <button
                                    ...
                                    disabled={isTranslatingAll || allTranslated}
                                ...
                                >
                                    ...
                                    {isTranslatingAll ? 'Translating...' : allTranslated ? 'All translated' : 'Translate all'}
                                </button>
                            )}
                                ...
                                <div className="space-y-6">
                                    {filteredTranscript.length === 0 ? (
                                        <p className="text-text-tertiary">No transcript available.</p>
                                            ...
                                            const isPending = translation?.state === 'pending';
                                            const hasTranslation = translation?.state === 'complete' && translation.text;
                                            return (
                                                <div key={i} className="group">
                                                    <div className="flex items-center justify-between mb-1">
                                                        ...
                                                        <button
                                                            ...
                                                            disabled={isPending}
                                                            className="opacity-0 group-hover:opacity-100 flex items-center gap-1 text-xs text-text-tertiary hover:text-text-secondary transition-all disabled:opacity-50"
                                                        >
                                                            {isPending ? <Loader2 size={12} className="animate-spin" /> : <Languages size={12} />}
                                                    ...
                                                    {hasTranslation && (
                                                        <div className="flex items-start gap-1.5 mt-1.5">
                                                            <Globe size={13} className="text-accent-primary mt-0.5 shrink-0" />
                                                            <p className="text-text-tertiary text-[14px] leading-relaxed select-text cursor-text">{translation.text}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            );
                                        ...
                                        {interaction.answer && (
                                            <div className="flex items-start gap-4">
                                                <div className="mt-1 w-6 h-6 rounded-full bg-bg-input flex items-center justify-center border border-border-subtle shrink-0">
                                                    <img src={pikaLogo} alt="AI" className="w-4 h-4 opacity-50 object-contain drop-shadow-sm" />
                                                </div>
                                                <div>
