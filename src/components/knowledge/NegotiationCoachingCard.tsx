  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg bg-surface-2 p-4">
      ...
      <div className="flex items-center gap-2">
        <span className="flex items-center gap-1.5 rounded-full bg-state-success-soft px-2.5 py-1 text-xs font-medium text-state-success">
        ...
        {phase.silence_strategy && (
          <span className="rounded-full bg-state-warning-soft px-2 py-1 text-xs text-state-warning">
            Silence OK
          </span>
        )}
      </div>
      ...
      {suggested_response && (
        <div className="rounded-lg bg-surface-3 p-3">
          <p className="mb-1 text-xs font-medium text-state-success">Suggested Response</p>
      ...
      {/* Warning */}
      {displayWarning && (
        <div className="flex items-start gap-2 rounded-lg bg-state-warning-soft p-3">
          <svg className="mt-0.5 h-4 w-4 flex-shrink-0 text-state-warning" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
          <p className="text-sm text-state-warning">{displayWarning}</p>
        </div>
      )}
      ...
      {showSilenceTimer && phase.silence_strategy && (
        <div className="flex items-center gap-2">
          ...
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-surface-3">
            <div
              className="h-full bg-state-warning-soft transition-all duration-1000"
